...
class WC_Deposits_Add_To_Cart
{
    ...
    public function enqueue_scripts()
    {
        ...
        $script_args = array(
            ...
            'message' => array(
                'deposit' => wp_kses(__($message_deposit, 'woocommerce-deposits'), $allowed_html),
                'full' => wp_kses(__($message_full_amount, 'woocommerce-deposits'), $allowed_html),
            )
        );
            ...
            case 'payment_plan':
                ...
                foreach($available_plans as $available_plan){
                    $plan_id = $available_plan->term_id;
                     $plan_name = $available_plan->name; 
                    $string = "your_string_here";
$durations = '';
if (strpos($plan_name, "6") !== false) {
     $durations = 2;
}
if (strpos($plan_name, "8") !== false) {
     $durations = 3;
}
if (strpos($plan_name, "10") !== false) {
     $durations = 4;
}

if (strpos($plan_name, "12") !== false) {
     $durations = 5;
}

if($durations!=''){
    $product_costs = get_post_meta($product_id,'_prowc_ppbf_param_1',true);
                //$durations = 6;
                $product_costs_duration = get_post_meta($product_id,'_prowc_ppbf_param_'.$durations,true);
                
                if($product_costs!='' && $product_costs_duration!=''){
                    
                    $modified_price = $product_costs*($product_costs_duration/100+1);
                }
    $price = $modified_price;
                    ...
                    $initial_deposit = get_post_meta($product_id,'_prowc_ppbf_param_'.$initial_dep,true);
                    if($initial_deposit!=''){
                        $plan_deposit_amount = $initial_deposit;
                    }
                      ...
                      $initial_deposit = get_post_meta($product_id,'_prowc_ppbf_param_'.$initial_dep,true);
                        ...
                        if($initial_deposit!=''){
                            $plan_deposit_percentage = $initial_deposit;
                        }
                        ...
                        if($initial_deposit!=''){
                            $total_plans= count($plan_payment_details['payment-plan']);
                            $line_percentage = (100-$initial_deposit)/$total_plans;
                        }
...
$plan_total_percentage = 100;
                    ...
                    foreach ($plan_payment_details['payment-plan'] as $key => $payment_line) {
                        ...
                        //    echo 'heree';
                            ...
                            // echo 'line percentage'.$line_percentage;
                        }
                        ...
                        if ($count === count($plan_payment_details['payment-plan'])) {
                            ...
                            if($initial_deposit!=''){
                                $line_amount = $plan_amount_type === 'fixed' ? round($line_percentage, wc_get_price_decimals()) : round($price / 100 * $line_percentage, wc_get_price_decimals());
                            }
                            else{
        ...
        if ($amount_type === 'fixed') {
            ...
            if ($product_type === 'booking' && method_exists($product,'has_persons') && $product->has_persons() && $deposits_enable_per_person === 'yes') {
                $suffix = esc_html__('per person', 'woocommerce-deposits');
            } elseif ($product_type === 'booking') {
                $suffix = esc_html__('per booking', 'woocommerce-deposits');
            } elseif (!$product->is_sold_individually()) {
                $suffix = esc_html__('per item', 'woocommerce-deposits');
            } else {
                $suffix = '';
            }
        ...
        $deposit_text = get_option('wc_deposits_button_deposit', esc_html__('Pay Deposit', 'woocommerce-deposits'));
        $full_text = get_option('wc_deposits_button_full_amount', esc_html__('Full Amount', 'woocommerce-deposits'));
        $deposit_option_text = get_option('wc_deposits_deposit_option_text', esc_html__('Deposit Option', 'woocommerce-deposits'));
