...
//            full payment item
    ...
    static function calculate_deposit_for_cart_item($cart_item, $item_amount_type, $amount_or_percentage = 0, $selected_plan = false, $plan_override = array(), $taxes_handling = '')
    {
     ...
     //   $plan_override = isset($cart_item['deposit']['override']) ?$cart_item['deposit']['override']:array();
        ...
        switch ($item_amount_type) {
            ...
            case 'payment_plan':
                ...
                // print_r($plan_override);
                // echo '<pre>';
                // print_r($plan_deposit_percentage);
                // die();
                ...
                $plan_name = $term->name;
                       ...
                       //echo $initial_dep.'<br />';
                      $initial_deposit = get_post_meta($product->get_id(),'_prowc_ppbf_param_'.$initial_dep,true);
                      if($initial_deposit!=''){
                        $plan_deposit_percentage = $initial_deposit;
                        $plan_override['deposit_percentage']=$initial_deposit;
                    }
...
if (!is_wp_error($term)) {
    ...
    // Determine the duration based on the term name
    $durations = '';
    if (strpos($plan_name, "6") !== false) {
        $durations = 2;
    } elseif (strpos($plan_name, "8") !== false) {
        $durations = 3;
    }elseif (strpos($plan_name, "10") !== false) {
        $durations = 4;
    } elseif (strpos($plan_name, "12") !== false) {
        $durations = 5;
    } 

    if (!empty($durations)) {
        // Retrieve the product costs and duration-specific costs
        $product_costs = get_post_meta($product->get_id(), '_prowc_ppbf_param_1', true);
        $product_costs_duration = get_post_meta($product->get_id(), '_prowc_ppbf_param_' . $durations, true);

        // Calculate the modified price using the provided formula
        if (!empty($product_costs) && !empty($product_costs_duration)) {
            ...
            $original_price = $modified_price;
                ...
                if ($enable === 'yes' && intval($plan_total_percentage) !== 100 && WC()->cart->cart_contents[$cart_item['key']]['data']->get_meta('_wcdp_recalculated') === 'yes') {
                    $original_price = $original_price / $plan_total_percentage * 100;
                }
